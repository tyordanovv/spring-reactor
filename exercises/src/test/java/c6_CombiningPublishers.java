...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        Flux<Void> tasks = taskExecutor()
                .flatMap(Function.identity());//runs every Mono<Void> in Flux<Mono<Void>>
    ...
    public void streaming_service() {
        Flux<Message> messageFlux = streamingService()
                .flatMapMany(Function.identity()); //runs flux in Mono<Flux<Message>>
    ...
    public void plan_b() {
        Flux<String> stonks =
//                Flux.firstWithValue(getStocksLocalCache(), getStocksRest());
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(
                StreamingConnection.startStreaming(), //supplies Flux from Mono
                n -> n,
                tr -> StreamingConnection.closeConnection());//cleanup after complete
